"""Text helpers shared by procedure generation and validation."""
...
SUBPROCESS_EXPANSION_PATTERN = re.compile(
    r"\b(?:incluant|comprenant|notamment|couvrant|"
    r"composé\s+de|compose\s+de)\b",
    flags=re.IGNORECASE,
)
...
def clean_sentence(text: str) -> str:
    ...
    if not sentence:
        raise ValueError(
            "A generated sentence cannot be empty."
        )
...
def light_stem(token: str) -> str:
    ...
    if stem.startswith("approvision"):
        return "approvision"
...
def looks_like_technical_title(
    title: str,
    process_id: str,
) -> bool:
    ...
    return (
        ...
        or normalized_title == str(process_id).strip()
        or normalized_title.startswith("Id_")
    )
